...
and freon cooling, its architecture is startlingly close to what you are about to build:
...
you have caches and a tight power budget.
...
> **Design lesson for MEDS-V.** Keep data in the VRF across multiple operations. Your
> benchmark kernels in Chapter 14 should be written to load once, compute several times,
> store once. A kernel that loads and stores for every operation is measuring your memory
> system, not your vector unit.

**4. Banked memory.** With no cache, the Cray-1 interleaved memory across many banks so a
unit-stride vector load could pull one element per cycle. Your VLSU faces the same problem
in modern dress: how do you get VLEN bits per cycle out of a memory system built for 64?
...
- **Intel MMX (1997)** — reused the x87 floating-point registers as 8×8-bit or 4×16-bit
  integers. Reusing the FP registers was so awkward you couldn't mix MMX and FP code
...
Compile for SSE, run on an AVX-512 machine: you get SSE performance. To benefit, someone
...
existing core (no new register file if you reuse FP registers, no `vl` state to save on
...
and lessons fed directly into the RVV working group. If you read one research paper
...
> **Check the version of anything you copy.** If you see `vsetvli` without `ta`/`ma`
> policy bits, or the mnemonics `vfredsum` (rather than `vfredusum`), you are looking at
> 0.7.x. This project targets **1.0 only.**

That RVA23 line is the commercial punchline, and it belongs in your project's motivation
...
You will meet all of these again in Chapter 4; meeting them here first, as *answers*, makes
them stick.
...
One binary, all implementations. You measured this in Chapter 1.
...
**Solution:** `LMUL` (vector register group multiplier) lets you glue 2, 4, or 8
architectural registers into one longer logical vector. With LMUL=8, `v8` means "v8 through
v15, as a single vector of 8×VLEN bits". You now have 4 usable registers, each 8× longer —
...
elements. Without a fix, you waste 3/4 of the source register, or need awkward
...
a table that looks bizarre until you know it exists to make mixed-width code natural.
...
**Cost:** register pressure on `v0`, and extra `vmv`/mask-move instructions when you juggle
several masks. A deliberate trade: simplicity of hardware over convenience of software.

**Implementation consequence for you:** your mask read port is *hardwired to `v0`*. That is
...
write on the destination register — you must preserve bits you aren't writing. That costs a
...
leave those elements alone *or* overwrite them with all-1s, whichever is cheaper. If your
implementation would rather blast a full-width write and not read the old value, agnostic
lets you.

**Implementation consequence — this is a big one.** If you support *only* agnostic
policies, your VRF write port never needs to merge with old data for tails, and vector
instructions have no false dependency on their destination register. If you support
*undisturbed*, you need a read-modify-write path. Appendix E's scope contract discusses
...
> **🎯 Milestone hook — pick your target name now.** MEDS-V v1 should aim at
> **`Zve32x` + `Zvl128b`** — 32-bit integer elements, VLEN ≥ 128, no floating point. That
> is a *real, standard, citable* extension name, achievable in a semester, and it means you
> can say "MEDS-V implements Zve32x_Zvl128b" rather than "MEDS-V implements some vector
> instructions we chose." The difference in how that reads in a report is enormous.
...
You will be compared against these, so know them. All are open source and all are worth
reading — see Appendix D for links.

| Design | Origin | Language | Style | Notes for you |
|---|---|---|---|---|
| **Ara / Ara2** | ETH Zürich | SystemVerilog | Long-vector, multi-lane, coupled to the CVA6 scalar core | The reference open RVV 1.0 design. 2–16 lanes. Read its lane structure. **Closest to what you are building.** |
...
| **Saturn** | UC Berkeley | Chisel | Short-vector, deeply parameterised, in-order decoupled | Excellent companion *technical report* on instruction scheduling. Read the document even if you don't read the Chisel. |
| **Spatz** | ETH Zürich | SystemVerilog | Very small vector unit for tightly-coupled clusters | Good if you care about area efficiency. |
| **AraXL** | ETH Zürich | SystemVerilog | Scaled-out Ara for very long vectors | Shows what happens when you push lane counts up. |
...
> each, and note the numbers in your project log. This calibrates your sense of the effort
> involved and is a sanity check on your own scope. Revisit in week 12.

---

## 3.6 Where MEDS-V fits

Put your own project on the map honestly. This table is a draft of a slide in your final
presentation:
...
**3.2** Explain, in your own words and without looking, why AVX-512 needed new instruction
...
**3.4** Fractional LMUL: you have 16-bit input data and want 64-bit accumulators, with the
same element count in both. What LMUL should the input use if the accumulator uses LMUL=2?
(Chapter 4 will give you the rule; try to reason it out first.)
...
- The Cray-1 got the architecture right in 1976: vector registers, a length register,
  chaining. Your design is a descendant.
